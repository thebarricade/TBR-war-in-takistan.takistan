...
{
	case "SAD" : { "<t color='#FFFF00' size='0.7' font='Zeppelin33Italic'>Unit will move to location, provide Close Air Support and engage all painted targets</t>" };
};
...
if (toUpper (_lb lbText _index) == "SAD") then
{
	...
	_casRadiusSliderText ctrlSetPosition [safeZoneX + (safeZoneW / 2.255), safeZoneY + (safeZoneH / 1.48), (safeZoneW / 11), (safeZoneH / 75)];
	_casRadiusSliderText ctrlCommit 0;
	_casRadiusSlider ctrlSetPosition [safeZoneX + (safeZoneW / 2.275), safeZoneY + (safeZoneH / 1.45), (safeZoneW / 10), (safeZoneH / 75)];
