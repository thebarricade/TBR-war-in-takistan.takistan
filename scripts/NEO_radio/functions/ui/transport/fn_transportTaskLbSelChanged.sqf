...
{
	case "PICKUP" : { "<t color='#FFFF00' size='0.7' font='Zeppelin33Italic'>Unit will move to location and wait for a smoke visual and confirmation (EVAC)</t>" };
	case "LAND" : { "<t color='#FFFF00' size='0.7' font='Zeppelin33Italic'>Unit will move to coordinates and land the chopper at his discression (DROP)</t>" };
	case "LAND (ENG OFF)" : { "<t color='#FFFF00' size='0.7' font='Zeppelin33Italic'>Unit will move to coordinates and land the chopper at his discression and will shutdown engine (DROP)</t>" };
	case "MOVE" : { "<t color='#FFFF00' size='0.7' font='Zeppelin33Italic'>Unit will move to designated position and wait for further orders</t>" };
	case "CIRCLE" : { "<t color='#FFFF00' size='0.7' font='Zeppelin33Italic'>Unit will move to designated corrdinates and circle the area clockwise till further notice</t>" };
};
...
{
	_slider ctrlSetPosition [safeZoneX + (safeZoneW / 2.275), safeZoneY + (safeZoneH / 1.601), (safeZoneW / 10), (safeZoneH / 60)];
	_slider ctrlCommit 0;
	_sliderText ctrlSetText "Radius: 200/300";
	_sliderText ctrlSetPosition [safeZoneX + (safeZoneW / 2.255), safeZoneY + (safeZoneH / 1.650), (safeZoneW / 10), (safeZoneH / 60)];
